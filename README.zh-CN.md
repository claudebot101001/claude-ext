...
> **21k 行代码** | **9 个扩展** | **654 项测试** | **纯 Python，零框架依赖**
...
这些核心原语驱动 9 个独立扩展，可通过配置文件自由启用/禁用。
...
| **browser** | 3 | 网页自动化（agent-browser CLI）+ 反爬虫抓取（Scrapling MCP 网关） |
...
|---|---|---|---|
| **代码量** | ~21k 行（Python） | ~800k 行（TypeScript） | ~3.9k 行（TypeScript） |

...
| **MCP 服务器注入** | 每会话独立 MCP 服务器，动态工具注册 |
...
这些核心原语驱动 8 个独立扩展，可通过配置文件自由启用/禁用。
...
| **memory** | 5 | 跨会话持久记忆。Markdown 文件 + FTS5 全文搜索 |
| **heartbeat** | 6 | 自主定期 Agent。三层执行（门控 → LLM 决策 → 会话）+ 五重安全阀 |
...
### vs. 多 Agent 编排器（Ruflo、Claude Squad、Overstory 等）

多数编排器聚焦于协调多个 Agent 完成开发任务。claude-ext 不同——它提供编排之下的**基础设施层**：
...
| **扩展体系** | 解耦生命周期 + 健康检查 | 单体架构 |

### vs. 官方 Claude Code 插件

[Claude Code 插件系统](https://docs.anthropic.com/en/docs/claude-code)在单个会话内扩展 Claude Code。claude-ext 在不同层面运作——管理**多个并发会话**及服务端基础设施（持久状态、凭证库、自主调度、跨会话协调）。两者互补：claude-ext 会话可以使用 Claude Code 插件。

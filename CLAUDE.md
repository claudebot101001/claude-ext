...
| `relative_time(iso_str)` | ISO 时间戳 | 人类可读的相对时间字符串（如 `"in 2h30m"`） |
...
自主心跳扩展。周期性唤醒 Claude 检查用户定义的任务清单，LLM 决定是否需要行动，仅在需要时创建完整 session 执行。

**架构：双通道调度 + 三层执行**

```
Timer Channel (周期定时 + 自适应退避)
  ↓
Scheduler Loop (asyncio.wait: 先到先处理)
  ↑
Trigger Channel (asyncio.Queue, 外部扩展实时事件)

Tier 0: 调度器门控 (Python, 零成本) → enabled? 利用率? 并发? 活跃时段?
Tier 1: 预检查 (Python) → HEARTBEAT.md 非空? 有待处理事件?
Tier 2: LLM 决策 (engine.ask(), ~500 token) → "NOTHING" 或任务描述
Tier 3: 执行 (完整 session, 仅当 Tier 2 决定需行动) → Telegram 流式投递
```

**静默抑制**：Tier 2 使用 `engine.ask()`（轻量子进程），不创建 tmux session，"NOTHING" 完全静默。仅 Tier 3 创建的 session 有 `chat_id`，Telegram 才投递。

**存储**：
- 状态：`{state_dir}/heartbeat/state.json`（JSON + flock）
- 指令：`{state_dir}/heartbeat/HEARTBEAT.md`（Markdown + flock）
- HeartbeatStore 是纯文件 I/O 层，不含 asyncio 对象（MCP server 进程也实例化）

**事件触发接口**（其他扩展通过 `engine.services["heartbeat"]` 调用）：
```python
...
`trigger()` 是 sync 方法。`immediate` 立即唤醒调度器；`normal` 积累到下次定时器到期。

**服务注册**：`engine.services["heartbeat"] = self`（注册扩展实例而非 store），因为服务接口包括 `trigger()`。

**利用率感知成本控制**：

| 利用率 | 定时器心跳 | Immediate 触发 |
|--------|-----------|----------------|
| < 80%  | 正常      | 正常           |
| 80-95% | 暂停      | 允许           |
| ≥ 95%  | 暂停      | 暂停           |

利用率通过 `core/status.py: get_usage()` 获取，60 秒缓存。

**自适应退避**（仅影响定时器通道）：

| 连续 NOTHING | 间隔倍数 | 示例（基准 30 分钟） |
|---|---|---|
| 0-3 | 1x | 30 分钟 |
| 4-6 | 2x | 1 小时 |
| 7-9 | 4x | 2 小时 |
...
| 工具 | 功能 |
|------|------|
| `heartbeat_get_instructions` | 读取 HEARTBEAT.md |
| `heartbeat_set_instructions` | 覆写 HEARTBEAT.md |
...
class HeartbeatState:
    enabled: bool = True
    last_run: str | None = None          # ISO（最近一次 Tier 2 调用）
    next_run: str | None = None          # ISO（下次定时器到期）
    run_count: int = 0                   # 总 Tier 2 调用数
    runs_today: int = 0                  # 今日 Tier 2 调用数
    runs_today_date: str | None = None   # YYYY-MM-DD
    consecutive_noop: int = 0            # 连续 NOTHING 次数
    active_session_id: str | None = None # 当前 Tier 3 session
```

**安全控制**：日运行上限 48、利用率节流 80%/暂停 95%、活跃时段窗口、并发限制 1、自适应退避最高 8x、完成后 5 秒自动清理 session。

**恢复检查**：`start()` 检查残留 `active_session_id`，若目标 session 为 DEAD/STOPPED/IDLE/不存在则清除，防止并发门控永久阻塞。

**事件日志**：`heartbeat.noop` / `heartbeat.decided` / `heartbeat.started` / `heartbeat.completed` / `heartbeat.skipped` / `heartbeat.triggered`。
...
extensions:                       # 每个扩展的独立配置
  ...
  heartbeat:
    interval: 1800                # 基准间隔秒数（默认 30 分钟）
    # active_hours: "08:00-22:00" # 可选 UTC 活跃窗口
    max_daily_runs: 48            # 日 Tier 2 决策上限
    usage_throttle: 80            # 利用率 ≥ 此值：仅允许 immediate 触发
    usage_pause: 95               # 利用率 ≥ 此值：全部暂停
    user_id: "123456789"          # 必填：所属用户 ID
    notify_context:               # 投递路由
      chat_id: 123456789
    working_dir: null             # 默认工作目录
  cron:

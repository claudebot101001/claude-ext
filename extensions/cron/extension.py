...
from core.session_context import get_extension_state, register_legacy_keys, set_extension_state
from core.session_tags import HEARTBEAT_RECLAIMABLE_TAG, SUBAGENT_RECLAIMABLE_TAG
...
_LEGACY_CONTEXT_KEYS = {
    "cron_job_id": ("cron", "job_id"),
    "cron_auto_cleanup": ("cron", "auto_cleanup"),
}
...
class ExtensionImpl(Extension):
    ...
    def configure(self, engine, config):
        ...
        register_legacy_keys(_LEGACY_CONTEXT_KEYS)
    ...
    async def start(self) -> None:
        ...
        self.sm.add_session_tagger(self._session_tags)
    ...
    def _session_tags(self, session) -> set[str]:
        if get_extension_state(session, "cron", "auto_cleanup"):
            return {HEARTBEAT_RECLAIMABLE_TAG, SUBAGENT_RECLAIMABLE_TAG}
        return set()
    ...
    async def _execute_new(self, job: CronJob) -> None:
        ...
        set_extension_state(context, "cron", "job_id", job.id)
        ...
        set_extension_state(session, "cron", "auto_cleanup", True)
    ...
    async def _execute_reuse(self, job: CronJob) -> None:
        ...
        if session:
            ...
            set_extension_state(session, "cron", "job_id", job.id)
    ...
    async def _reclaim_cron_session(self, user_id: str) -> bool:
        ...
        for s in self.sm.get_sessions_for_user(user_id):
            if get_extension_state(s, "cron", "auto_cleanup") and s.status in (
                SessionStatus.IDLE,
    ...
    async def _on_delivery(self, session_id: str, result_text: str, metadata: dict) -> None:
        ...
        if not session:
            return
        job_id = get_extension_state(session, "cron", "job_id")
        if not job_id:

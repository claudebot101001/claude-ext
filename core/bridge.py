...
class BridgeServer:
    ...
    async def dispatch(self, method: str, params: dict | None = None) -> dict:
        """Internal dispatch: call registered handlers from the main process.

        Same semantics as an external bridge call but without going through
        the Unix socket.  First non-None handler response wins.
        """
        for handler in self._handlers:
            try:
                result = await handler(method, params or {})
            except Exception:
                log.exception("Bridge dispatch error for method=%s", method)
                return {"error": f"Handler error for {method}"}
            if result is not None:
                return result
        return {"error": f"No handler for method: {method}"}

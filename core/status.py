...
async def get_usage() -> dict:
    ...
    Error responses are NOT cached so the next call retries.
    """
    global _usage_cache, _usage_cache_ts

    now = time.monotonic()
    if _usage_cache is not None and (now - _usage_cache_ts) < _USAGE_CACHE_TTL:
        return _usage_cache
    ...
    # Only cache successful responses (have utilization data, no error)

...
- **mcp_server.py**: `heartbeat_get_instructions` / `heartbeat_set_instructions` / `heartbeat_get_status` / `heartbeat_pause` / `heartbeat_resume` / `heartbeat_trigger` / `heartbeat_get_trigger_command` seven MCP tools. First five use direct file I/O (same pattern as memory), `heartbeat_trigger` uses bridge RPC to call main process `trigger()` method, `heartbeat_get_trigger_command` returns a shell command usable by external scripts

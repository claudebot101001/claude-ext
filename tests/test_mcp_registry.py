...
class TestRegisterMCPServer:
    ...
    def test_registers_server_tags(self, sm):
        sm.register_mcp_server(
            "vault",
            {"command": "python"},
            tags=["read_only_worker_exclude", "sensitive"],
        )
        assert sm.get_mcp_server_tags("vault") == {"read_only_worker_exclude", "sensitive"}

    def test_get_mcp_servers_by_tag(self, sm):
        sm.register_mcp_server(
            "vault",
            {"command": "python"},
            tags=["read_only_worker_exclude"],
        )
        sm.register_mcp_server(
            "browser",
            {"command": "python"},
        ...
        sm.register_mcp_server("memory", {"command": "python"}, tags=["knowledge"])

        assert sm.get_mcp_servers_by_tag("read_only_worker_exclude") == {"vault", "browser"}
        assert sm.get_mcp_servers_by_tag("knowledge") == {"memory"}

    def test_tag_accessors_return_copies(self, sm):
        sm.register_mcp_server(
            "vault",
            {"command": "python"},
            tags=["read_only_worker_exclude"],
        ...
        tags = sm.get_mcp_server_tags("vault")
        tags.add("mutated")

        assert sm.get_mcp_server_tags("vault") == {"read_only_worker_exclude"}
